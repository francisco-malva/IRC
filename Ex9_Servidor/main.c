...
int main(int argc, char* argv[])
{
	...
	while (1) {
		...
		nbytes = recvfrom(sockfd, buffer, sizeof(buffer), 0, &from, &fromLen);
		...
		int nullTerm = msg->Length;
		msg->Data[nullTerm] = '\0';

		printf("\n<SER1> Mensagem recebida {%s} de %s, a reenviar.\n", msg->Data, inet_ntoa(from.sin_addr));

		if (sendto(sockfd, buffer, nbytes, 0, &from, fromLen) == SOCKET_ERROR)
			Abort("Falha no envio.");
		else
			printf("<SER1> Enviado com sucesso!\n");
